...
const MODE_TOOLS: Record<ChatMode, readonly string[] | undefined> = {
  ...
  website_info: ["search_pages", "list_categories"],
  blogs: ["search_pages"],
...
- 2 to 20 matches: describe them in a short, natural paragraph or list, using the EXACT names, prices, and SKUs from the tool result — never guess or invent a detail that isn't in the data. Invite the user to ask for more on any one of them, or to narrow further. Don't call get_product_details yet unless the user has already named a specific one from the list.
- More than 20 matches: don't list individual products. Ask ONE clarifying question built directly from the \`facets\` field in the tool result — name the actual top 3-4 categories (or brands) and their counts, e.g. "I found 62 tools — want to narrow by category? Cordless Power Tools (62), Drills & Fastening (12), Saws & Cutters (5)?" EXCEPTION: if the user's message shows they already answered a narrowing question and results are still large (e.g. they just gave a category/feature and it's still 20+), or they explicitly ask to just see the list/all of them, don't ask again — describe what you have, even if that means a longer list. Never ask the same kind of narrowing question two turns in a row.
...
const MODE_INSTRUCTIONS: Record<ChatMode, string> = {
  ...
  website_info: `You are in WEBSITE INFO mode. Use search_pages to answer questions about the site's content, policies, shipping/returns, and company info — trust each result's \`contentSnippet\` over its \`excerpt\` for specific facts (an email, a link, a number), since the excerpt is just short marketing copy and often won't contain them. If the user asks what product categories/types exist, call list_categories instead — never guess a category list from general knowledge. If the user's message is just a generic opener with no actual question yet, ask what they'd like to know rather than searching for nothing. If search_pages comes back empty, say plainly that you couldn't find that on the site rather than guessing an answer.`,
  blogs: `You are in BLOG SEARCH mode. Only use search_pages (already restricted to blog posts) to find and summarize relevant articles. If the user's message is just a generic opener or has no specific topic (e.g. "show me recent posts", "what's new on the blog"), call search_pages with no \`query\` at all — that returns the most recently published posts, which is exactly what they're asking for. Only ask a clarifying question if they seem to want something specific you can't infer.`,
