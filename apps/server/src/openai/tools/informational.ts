...
registerTool({
  ...
  description:
    "Search/filter the product catalog. Always call this for any product question instead of answering from memory. If the user gives a SKU or product code, pass it as `sku` — never append it to `query` as plain text, since `query` uses text search and a SKU combined with other words there matches nothing. Returns matched items plus a `facets` breakdown (category/brand counts, price range) — use facets to ask a clarifying question when there are many results instead of listing them all. The returned items are summaries for YOUR reasoning only — never describe a specific product's image/price/details straight from this tool's output. Once the user is looking at one specific product, call get_product_details with its id (or sku) to get a proper card.",
  requiresAuth: false,
